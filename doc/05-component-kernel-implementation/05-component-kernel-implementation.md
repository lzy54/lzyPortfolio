...
I chose Map as my representation. Because the characteristic of a Map is that it corresponds two data structures, it very much fits my needs. Although there will always be only one pair in the Map, it is still the best choice. I can store the word that 'this' itself represents at the key's position, and the value is a sequence<Strings> to store example sentences containing this word.
...
kernel methods implementation is easy because we've already done several times. However, implementing Standard methods is a big challenge for me.
